...
export class AuthService {
    ...
    async signUp(credentials: SignUpCredentials): Promise<void> {
        const password = await this.passwordHasherService.hashPassword(credentials.password);
    ...
    async verifyCredentials(credentials: VerifyCredentials): Promise<UserProfile> {
        const user = await this.usersService.findOne(credentials);

        if (!user) {
            throw new NotFoundException('There are not registered users with this email or name!');
        }
